...
public class ManageFlights extends Options{
    ...
    public void insertFlight() throws ParseException {
        ...
        Program.flightRepository.create(flight);
    }
    ...
    public void loadFlights(Map<Integer, List<String>> data) throws ParseException {
        ...
        for (Integer key : data.keySet()) {
            ...
            Location origin = new Location(data.get(key).get(3),data.get(key).get(4));
            Location destination = new Location(data.get(key).get(5),data.get(key).get(6));
            ...
            Date departure = def.parse(targetFormat.format(formatter.parse(data.get(key).get(7))));
            Date arrival = new SimpleDateFormat("EEE MMM dd HH:mm:ss zzzz yyyy", Locale.ENGLISH).parse(data.get(key).get(8));
